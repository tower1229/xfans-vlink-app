...
export default function Signup() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 表单验证
        if (!formData.username || !formData.walletAddress || !formData.password || !formData.confirmPassword) {
            setError("请填写所有必填字段");
            setLoading(false);
            return;
        }
        ...
        if (!walletRegex.test(formData.walletAddress)) {
            setError("请输入有效的钱包地址（以太坊格式：0x开头，后跟40个十六进制字符）");
            setLoading(false);
            return;
        }
        ...
        try {
            // 调用注册API
            const success = await register(formData.username, formData.walletAddress, formData.password);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={loading}
                        />
                        <p className="mt-1 text-xs text-gray-500">用于接收通过Vilink协议的Web3支付</p>
                    </div>
